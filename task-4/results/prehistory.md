...
2.  Флоу
    ...
    ![](load-csv-diagram.png) Пояснения к схеме:
    ...
    2.  Пользователь запускает загрузку полученного CSV-файла через интерфейс ERP:
        ...
        2.  В случае ошибки пользователь получает ответ в браузере: «Ошибка формата загружаемых данных».

        1 Файл сохраняется в хранилище в GCS.
        1.  Запускается процесс
            1.  обработки файла,
            2.  обновление значениями из БД справочных данных
            3.  и сохранения данных в БД номенклатуры.
        2.  Если операция обработки завершается успешно, то пользователь получает уведомление об этом.
